...
fn hand_type(s: &str) -> usize {
    let mut bags = s.chars().fold(HashMap::new(), |mut acc: HashMap<char, usize>, c| {
        acc.entry(c).and_modify(|e| *e += 1).or_insert(1);
        acc 
    });

    // cursed
    let num_jokers = *bags.get(&'J').unwrap_or(&0); 
    if num_jokers != 5 {
        bags.remove(&'J');
        let highest = bags.iter()
            .max_by(|(_, a), (_, b)| a.cmp(b))
            .map(|(k, _)| k)
            .unwrap();
        bags.entry(*highest).and_modify(|e| *e += num_jokers);
    }
...
fn beats_other(s: &str, other: &str) -> bool {
    let ranks: HashMap<char, usize> = HashMap::from_iter("J23456789TQKA".chars().zip(1..)); 
